...
@Component
public class TomTotCriticDeliberationPolicy implements DeliberationPolicy {
    ...
    public TomTotCriticDeliberationPolicy(StructuredModelGateway structuredModelGateway,
    ...
    public String policyId() {
        return AgentPolicyIds.TOM_TOT_CRITIC_V1;
    }
    ...
    public TurnAgentResult execute(PlayerTurnContext context, PlayerAgentConfig config) {
        ...
        try {
            ...
            throw new AgentTurnExecutionException(
                    "tom-tot-critic-v1 belief stage failed",
                    beliefRequest,
                    null,
                    attempts,
        ...
        try {
            ...
            throw new AgentTurnExecutionException(
                    "tom-tot-critic-v1 tot stage failed",
                    totRequest,
                    null,
                    ...
                    failedPolicySummary(
                            totRequest,
                            beliefAttempts + attempts,
                            support.modelSlotIds(beliefRequest, totRequest),
        ...
        String criticSlotId = support.hasExplicitModelSlot(effectiveConfig, "critic") ? "critic" : "actor";
        AgentTurnRequest criticRequest = requestFactory.create(context, effectiveConfig, criticSlotId);
        criticRequest.setMemory(support.mergeBeliefIntoMemory(criticRequest.getMemory(), beliefStageResult));
        criticRequest.setPromptText(tomPromptFactory.buildCriticUserPrompt(
                criticRequest,
                beliefStageResult,
                totStageResult
        ));

        StructuredStageRetryPolicy.StructuredStageExecution criticExecution;
        StructuredInferenceResult criticStructuredResult;
        TomCriticStageResult criticStageResult;
        Map<String, Object> criticTrace;
        int criticAttempts = 0;
        try {
            criticExecution = structuredStageRetryPolicy.execute("critic-stage", support.structuredRequest(
                    criticRequest,
                    tomPromptFactory.buildCriticDeveloperPrompt(criticRequest),
                    criticRequest.getPromptText()
            ), structuredModelGateway);
            criticStructuredResult = criticExecution.result();
            criticAttempts = criticExecution.attempts();
            criticStageResult = support.parseCriticStage(criticStructuredResult, policyId());
            criticTrace = support.stageTrace(
                    "critic-stage",
                    "structured-inference",
                    "COMPLETED",
                    criticRequest,
                    criticAttempts,
                    criticStructuredResult.getModelMetadata(),
                    criticTraceExtras(criticStageResult)
            );
        } catch (RuntimeException exception) {
            RuntimeException failure = exception instanceof StructuredStageRetryPolicy.StructuredStageExecutionException structuredException
                    ? structuredException.failure()
            ...
            int attempts = exception instanceof StructuredStageRetryPolicy.StructuredStageExecutionException structuredException
                    ? structuredException.attempts()
                    : Math.max(criticAttempts, 1);
            throw new AgentTurnExecutionException(
                    "tom-tot-critic-v1 critic stage failed",
                    criticRequest,
                    null,
                    beliefAttempts + totAttempts + attempts,
                    List.of(
                            beliefTrace,
                            totTrace,
                            support.failedStageTrace(
                    ...
                    failedPolicySummary(
                            criticRequest,
                            beliefAttempts + totAttempts + attempts,
                            support.modelSlotIds(beliefRequest, criticRequest),
                            beliefStageResult.getBeliefsByPlayerId().size(),
                            "critic-stage",
        ...
        try {
            ...
            return new TurnAgentResult(
                    ...
                    enrichedTurnResult,
                    validated.action(),
                    beliefAttempts + totAttempts + criticAttempts + validated.attempts(),
                    policyId(),
                    ...
                    successPolicySummary(
                            ...
                            criticAttempts,
                            validated.attempts(),
                            support.modelSlotIds(beliefRequest, criticRequest, validated.request())
                    )
            );
            ...
            throw new AgentTurnExecutionException(
                    ...
                    exception.request(),
                    exception.lastTurnResult(),
                    beliefAttempts + totAttempts + criticAttempts + exception.attempts(),
                    executionTrace,
                    failedPolicySummary(
                            exception.request(),
                            beliefAttempts + totAttempts + criticAttempts + exception.attempts(),
                            support.modelSlotIds(beliefRequest, criticRequest, exception.request()),
                            beliefStageResult.getBeliefsByPlayerId().size(),
                            "decision-stage",
                            totStageResult.getCandidates().size(),
                            totStageResult.getSelectedCandidateId(),
                            criticStageResult.getStatus(),
                            exception.getCause()
                    ),
                    exception.getCause()
            );
    ...
    private Map<String, Object> successPolicySummary(AgentTurnRequest decisionRequest,
        ...
        Map<String, Object> summary = support.baseSuccessPolicySummary(
                ...
                decisionRequest,
                4,
                beliefAttempts + totAttempts + criticAttempts + decisionAttempts,
                modelSlotIds,
                beliefStageResult.getBeliefsByPlayerId().size(),
                support.tokenValue(beliefStructuredResult.getModelMetadata())
                        + support.tokenValue(totStructuredResult.getModelMetadata())
                        + support.tokenValue(criticStructuredResult.getModelMetadata())
                ...
                support.outputTokenValue(beliefStructuredResult.getModelMetadata())
                        ...
                        + support.outputTokenValue(criticStructuredResult.getModelMetadata())
        ...
        summary.put("criticStatus", criticStageResult.getStatus());
        return summary;
    }
    ...
    private Map<String, Object> failedPolicySummary(AgentTurnRequest request,
        ...
        if (criticStatus != null) {
            summary.put("criticStatus", criticStatus);
        }
        return summary;
    }
    ...
    private Map<String, Object> criticTraceExtras(TomCriticStageResult criticStageResult) {
        Map<String, Object> extras = new LinkedHashMap<>();
        if (criticStageResult.getStatus() != null) {
            extras.put("criticStatus", criticStageResult.getStatus());
        }
        return extras;
    }
}
